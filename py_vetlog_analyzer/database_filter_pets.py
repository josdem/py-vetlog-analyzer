...
class PetFilter:
    ...
    def filtering_pets(self):
        self.cursor.execute("select pet.id, pet.name, pet.birth_date from pet, vaccination where vaccination.pet_id = pet.id and vaccination.status='PENDING' group by pet.id")
